package com.just.ksim.urbancomputing.endpoint;
            ...
            do {
                ...
                for (Cell v : results) {
                    if (Double.MAX_VALUE != currentThreshold) {
                        if (Bytes.toString(CellUtil.cloneQualifier(v)).equals(Constants.START_POINT) && request.getFunc() == 0) {
                        ...
                        } else if (Bytes.toString(CellUtil.cloneQualifier(v)).equals(Constants.END_POINT) && request.getFunc() == 0) {
